...
## Что `prune` реально делает с контекстом

`prune` — это не сворачивание в UI и не пометка «неважное», а настоящий вырез: диапазон от цитаты до текущего момента физически удаляется из живого контекста сессии и заменяется дистиллятом. Так и задумано — не побочный эффект, а весь смысл инструмента. Если после отката в контексте не находится прошлая реплика (своя или пользователя), которую ты ожидал там увидеть, — это почти всегда штатная работа `prune`, а не глюк модели или чужая галлюцинация.

Проверяй это через `graft`, а не через переписывание набело. Прежде чем заявить пользователю «этого не было» или «я что-то напутал» про собственный прошлый ход — сначала `graft(rule_id)` (см. [tz.md §6.5](../../../tz.md#65-grafttarget)), сверься с оригиналом, и только потом делай вывод. Дистиллят создан именно для такого восстановления: всё ценное из вырезанного диапазона там есть.
