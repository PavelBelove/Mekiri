...
- **Не наблюдаемо в потоке сообщений** — см. §3.2: `SDKHookResponseMessage` не эмитится для этого типа хука в принципе, это не баг, а архитектурное ограничение.

### 3.2. Почему живой smoke-тест не может ловить `hook_response` (второй, более узкий гэп)

Первая попытка Task 3 (rework) на `UserPromptSubmit` обнаружила: `additionalContext` реально доходит до модели (подтверждено маркер-фразой в реальном прогоне — модель дословно повторила инжектированную строку), но `SDKHookResponseMessage` (`subtype:'hook_response'`) **никогда не эмитится** для in-process JS callback-хуков — декомпиляция бандла CLI показала, что эта конструкция единственный раз строится функцией, читающей `shellCommand`-вывод (stdout/stderr/exit code) внешнего процесса; ветка, диспетчеризующая JS-callback-хуки (единственная форма, которую поддерживает `Options.hooks`), результат `additionalContexts` инжектит напрямую и никогда не строит `hook_response`. Это архитектурное ограничение наблюдаемости, не связанное с апстрим-багом `SessionStart` из §3.1 (там хук вообще не вызывался; здесь — вызывается и работает, просто не даёт отдельного сигнального сообщения в потоке).

**Следствие для тестирования:** живое доказательство того, что хук сработал, должно идти через поведение модели (она реально использовала `additionalContext`), а не через системное сообщение о хуке — см. §5.
...
- **Живой поведенческий smoke-тест** (не `hook_response`-сообщение — см. §3.2): реальная сессия с заранее подготовленным во временном каталоге «плохим» `audit.jsonl`, промпт, прямо просящий модель сообщить, есть ли у неё сейчас в контексте какой-то ожидающий tuning-сигнал от Mekiri и что именно он говорит — без подсказки самого ответа. Проверка, что реальный текстовый ответ модели содержит ожидаемые цифры/формулировку («Distillation Ratio»). Тот же приём, что уже подтверждён в этой же итерации (маркер-фраза `ZEBRA-QUARTZ-91`) и что уже использовался в живом поведенческом тесте system-prompt-плана («proactively names mekiri-gate»).
